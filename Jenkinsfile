    ...
    stage('Deploy') {
      steps {
        sh '''
          ...
          echo "Doing curl https://qwde.no:9000/hooks/qwde-deploy-simple?service=blog-download&token=..."
          curl "https://qwde.no:9000/hooks/qwde-deploy-simple?token=$token&service=blog-download"
          set -x
          '''
      }
    }
